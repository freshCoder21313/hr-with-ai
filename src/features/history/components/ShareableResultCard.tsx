  ...
  ({ interview }, ref) => {
    ...
    return (
      ...
      >
        {/* Background Elements */}
        <div
          className={`absolute top-0 right-0 w-64 h-64 bg-gradient-to-br ${gradient} opacity-20 blur-3xl rounded-full -mr-16 -mt-16`}
        />
        ...
        <div className="relative z-10 flex flex-col h-full justify-between">
          ...
          <div className="flex justify-between items-start">
            ...
            <div className="flex flex-col items-center">
              <div
                className={`relative flex items-center justify-center w-20 h-20 rounded-full bg-slate-900 border-4 border-transparent bg-clip-padding`}
              >
                <div
                  className={`absolute inset-0 rounded-full bg-gradient-to-br ${gradient} -z-10 m-[-4px]`}
                />
          ...
          <div className="grid grid-cols-2 gap-4 mt-4">
            <div className="space-y-3">
              <h3 className="text-xs uppercase tracking-wider text-white/50 font-semibold mb-1">
                Top Strengths
              </h3>
              <div className="flex flex-wrap gap-2">
                {strengths.slice(0, 3).map((strength, i) => (
                  <div
                    key={i}
                    className="flex items-center gap-1.5 bg-white/10 px-3 py-1.5 rounded-lg text-sm border border-white/5"
                  >
                  ...
                  <div className="flex flex-wrap gap-2">
                    {badges.slice(0, 2).map((badge, i) => (
                      <div key={i} className="flex flex-col items-center gap-1">
                        <div
                          className={`w-10 h-10 rounded-full bg-gradient-to-br ${gradient} flex items-center justify-center shadow-lg`}
                        >
                          <Trophy size={18} className="text-white" />
                        </div>
                        <span className="text-[10px] text-white/80 text-center max-w-[80px] leading-tight">
                          {badge}
                        </span>
                      </div>
                    ))}
