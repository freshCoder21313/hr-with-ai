  ...
  return (
    <div className={`rounded-lg border bg-card transition-all ${
      ...
      <div className="flex items-center gap-2 px-3 py-2">
        <Checkbox checked={isSelected} onCheckedChange={(c) => onSelect(!!c)} disabled={isDone || isProcessing} className="shrink-0" />
        ...
        <div className="flex items-center gap-1 shrink-0">
          ...
          <Button variant="ghost" size="icon" className="h-6 w-6 text-muted-foreground hover:text-destructive" onClick={onRemove} disabled={isProcessing}>
        ...
        <div className="px-3 pb-3 space-y-2 border-t border-border pt-2">
          <div className="grid grid-cols-2 gap-2">
            <div>
              ...
              <Input className="h-7 text-xs" placeholder="Google" value={job.company}
                onChange={(e) => onChange('company', e.target.value)} disabled={isDone || isProcessing} />
            </div>
            <div>
              <Label className="text-[10px]">Title</Label>
              <Input className="h-7 text-xs" placeholder="SWE" value={job.title}
                onChange={(e) => onChange('title', e.target.value)} disabled={isDone || isProcessing} />
            </div>
          </div>
          <div>
            <Label className="text-[10px]">Job Description</Label>
            <Textarea className="min-h-[80px] text-xs font-mono" placeholder="Paste JD here..."
              value={job.description} onChange={(e) => onChange('description', e.target.value)} disabled={isDone || isProcessing} />
          </div>
          <div>
            <Label className="text-[10px]">Custom Prompt (optional)</Label>
            <Textarea className="min-h-[48px] text-xs" placeholder="Emphasize React experience..."
              value={job.customPrompt} onChange={(e) => onChange('customPrompt', e.target.value)} disabled={isDone || isProcessing} />
          </div>
          {isDone && status.resultId && (
            <Button size="sm" variant="secondary" className="w-full h-7 text-xs" onClick={() => onViewResult(status.resultId!)}>
              View Tailored CV →
            </Button>
          )}
        </div>
