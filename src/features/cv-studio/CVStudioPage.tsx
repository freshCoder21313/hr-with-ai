...
const CVStudioPage: React.FC = () => {
  ...
  const [template, setTemplate] = useState<
    'modern' | 'classic' | 'creative' | 'minimalist' | 'academic'
  >('modern');
  ...
  const handleAcceptChange = async (change: ProposedChange) => {
    ...
    if (current !== undefined && isArrExp !== isArrRec) {
      alert(`Type mismatch for ${change.section}`);
      return;
    }
    if (current === undefined && !ALLOWED_SECTIONS.includes(change.section)) {
      alert(`Invalid section: ${change.section}`);
      return;
    }
  ...
  const handleStartTailoring = async () => {
    ...
    if (!sourceResume) {
      alert('Please select a source resume.');
      return;
    }
    ...
    if (!sourceData) {
      try {
        ...
        alert('Failed to parse source resume.');
  ...
  // ── Derived ─────────────────────────────────────────────────────────────────
  const previewData =
    mainCV?.parsedData ?? resumes.find((r) => r.id === selectedResumeId)?.parsedData;
  ...
  return (
    <>
      ...
      <div className="hidden print:block" style={{ width: '210mm', margin: '0', padding: '0' }}>
        {previewData && (
          <ResumePreview data={previewData} template={template} onUpdate={handleManualUpdate} />
        )}
      </div>
      ...
      <div className="flex h-[calc(100vh-64px)] overflow-hidden bg-background print:hidden">
            ...
            <div className="flex flex-col items-center pt-3 gap-2 px-1">
              ...
              {selectedJobs.size > 0 && (
                <Tooltip delayDuration={0}>
                  ...
                  <TooltipContent side="right">
                    Run Tailoring ({selectedJobs.size} jobs)
                  </TooltipContent>
                </Tooltip>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="relative flex items-center group">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setContextResumeId(e.target.value ? Number(e.target.value) : undefined)
                  }
                    ...
                    <div className="max-w-2xl mx-auto">
                      {activeTab === 'basics' && (
                        <BasicsForm
                          ...
                          onChange={(v) => handleManualUpdate({ ...mainCV.parsedData!, basics: v })}
                        />
                      )}
                      {activeTab === 'work' && (
                        <WorkForm
                          data={mainCV.parsedData.work}
                          onChange={(v) => handleManualUpdate({ ...mainCV.parsedData!, work: v })}
                        />
                      )}
                      {activeTab === 'education' && (
                        <EducationForm
                          ...
                          onChange={(v) =>
                            handleManualUpdate({ ...mainCV.parsedData!, education: v })
                          }
                        />
                      )}
                      {activeTab === 'skills' && (
                        <SkillsForm
                          ...
                          onChange={(v) => handleManualUpdate({ ...mainCV.parsedData!, skills: v })}
                        />
                      )}
                      {activeTab === 'projects' && (
                        <ProjectsForm
                          ...
                          onChange={(v) =>
                            handleManualUpdate({ ...mainCV.parsedData!, projects: v })
                          }
                        />
                      )}
                    </div>
        ...
        <SectionReorderDialog
          ...
          data={
            previewData
              ? { ...previewData, meta: { ...previewData.meta, template } }
              : {
          ...
          onSave={(newOrder) => {
            ...
            handleManualUpdate({
              ...mainCV.parsedData,
              meta: { ...mainCV.parsedData.meta, sectionOrder: newOrder, template },
            });
          }}
        />
